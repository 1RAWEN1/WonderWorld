...
public class ShieldButton extends Actor {
    private final TextureRegion texture = new TextureRegion(new Texture("buttons\\shield.png"));
    ...
    public void act(float delta) {
        if(Gdx.input.isTouched(Joystick.index) && touchThis()){
            GameScreen.player.setDef(true);
        }

        setPosition(GameScreen.player.getX() + 520, GameScreen.player.getY() + GameScreen.player.getZ() - 100);

        setVisible(GameScreen.player.getInventory().getItems()[5] != null && !GameScreen.player.isSeeInventory() && GameScreen.dialogReader.getTalker() == null);
    }
    ...
    public void draw(Batch batch, float parentAlpha) {
        if(GameScreen.isHelpLabelVisible() && GameScreen.getButIndex() == -2){
