...
public class Backpack extends Actor implements ButtonInterface {
    private final TextureRegion texture = new TextureRegion(new Texture("buttons\\backpack1.png"));
    ...
    public void act(float delta) {
        if(Gdx.input.isTouched(Joystick.index) && !tabPush){
            if(touchThis())
                GameScreen.player.setSeeInventory(!GameScreen.player.isSeeInventory());
            tabPush = true;
        }
        ...
        setPosition(GameScreen.player.getX() + 400, GameScreen.player.getY() + GameScreen.player.getZ() - 290);
    }
